...
def xunit_from_report(xunit, summary):
    ...
    with open(xunit, "w", encoding="utf8") as xml_file:
...
def get_preview_report(intermediate_dir,
    ...
    if text_output:
        with open(text_output, 'w', encoding="utf8") as dest:
            dest.write(
...
disa-stig:   
    ...
    version: V5
"""
    with open("_covreport.yml", "w", encoding="utf8") as file:
        file.write(cfg)
...
def run(args):
    ...
    error_count = 0
    with open(json_report_path, "r", encoding="utf8") as json_file:
    ...
    with open("_snapshot_id.txt", "r", encoding="utf8") as snapshot_id_file:
